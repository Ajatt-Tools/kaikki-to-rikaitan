...
function updateDownloadLink(tgtSel, glossSel, linkSel, type) {
    ...
    let url
    if (type === 'main') {
        url = `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${tgt}-${gloss}.zip`
    } else if (type === 'ipa') {
        url = gloss === 'merged'
            ? `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${tgt}-ipa.zip`
            : `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${tgt}-${gloss}-ipa.zip`
    } else if (type === 'translations') {
        url = `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${tgt}-${gloss}-gloss.zip`
    }
        ...
        const rows = allLangs.map(toLang => {  // allLangs = "to"
            ...
            for (const fromLang of glosses) {
                if (fromLang.iso === toLang.iso) {
                    ...
                    const url = `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${fromLang.iso}-${toLang.iso}-gloss.zip`
    ...
    const rows = allLangs.map(rowLang => {
        ...
        for (const gloss of glosses) {
            ...
            if (glossIso === 'merged' && type === 'ipa') {
                const url = `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${rowLang.iso}-ipa.zip`
                row.push(`<a href="${url}" target="_blank">📥</a>`)
            } else if (type === 'ipa') {
                const url = `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${rowLang.iso}-${glossIso}-ipa.zip`
                row.push(`<a href="${url}" target="_blank">📥</a>`)
            } else {
                const url = `https://github.com/Ajatt-Tools/kaikki-to-rikaitan/releases/latest/download/kty-${rowLang.iso}-${glossIso}.zip`
                row.push(`<a href="${url}" target="_blank">📥</a>`)
            }
        }
        return row
