...
function convertToRikaitan(sourceJson) {
...
if(!file){
  console.log("no file name provided, run with: node migaku-to-rikaitan.js <file>.json");
  process.exit();
}
...
readFile(file, 'utf8', (err, sourceJson) => {
  ...
  const converted = convertToRikaitan(sourceJson);
